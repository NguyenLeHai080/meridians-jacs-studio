...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Tối ưu hóa giao diện Responsive toàn diện cho màn hình nhỏ và laptop, Sửa lỗi tràn chữ trên thanh điều hướng Navbar & Marquee bản quyền, Nâng cao trải nghiệm người dùng."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.29)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.29&channel=stable'")
