...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Bổ sung tính năng phân tích đồng bộ Tiêu đề Video (Viral Title) & Nội dung kịch bản toàn diện, tối ưu hoá tải video YouTube URL chống chặn bot."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.23)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.23&channel=stable'")
