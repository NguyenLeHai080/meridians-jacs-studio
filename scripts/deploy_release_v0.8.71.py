...
VERSION = "0.8.71"
NOTES = "Bản cập nhật v0.8.71: Tích hợp Hệ thống AI Vocal & Stem Separator độc lập — Trích xuất sóng âm, bóc tách và loại bỏ 100% nhạc nền gốc (BGM) ra khỏi video trước khi cắt ghép phân cảnh, giữ trọn vẹn lời thoại nhân vật, tiếng còi hú cảnh sát, tiếng súng và tiếng động hiện trường."
...
if os.path.exists(os.path.dirname(LOCAL_INSTALLED_ASAR)):
    try:
        ...
        # Also copy voice-runtime to resources if needed
        local_voice = r"D:\Program Files\JACS Studio\resources\voice-runtime"
        os.makedirs(local_voice, exist_ok=True)
        shutil.copy2(os.path.join(APP_DIR, "voice-runtime", "voice_worker.py"), os.path.join(local_voice, "voice_worker.py"))
