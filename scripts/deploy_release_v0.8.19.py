...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Đồng bộ phân cảnh AI & Voice 1:1, khắc phục lỗi giật nhảy video 60 FPS trên Timeline, sửa lỗi Render giọng đọc AI (atempoChain), tối ưu hóa hiệu suất xuất video."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.18)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.18&channel=stable'")
