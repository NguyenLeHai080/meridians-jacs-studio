...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Kích hoạt đầy đủ Engine Render GPU NVIDIA NVENC / Apple VideoToolbox / QSV / AMF theo cài đặt ưu tiên, hiển thị trạng thái Render GPU trực tiếp."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.20)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.20&channel=stable'")
