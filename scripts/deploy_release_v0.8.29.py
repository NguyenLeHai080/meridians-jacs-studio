...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Tối ưu hóa toàn diện module Quản Lý Credits & Mức Dùng AI, Tắt chế độ tự động làm mới ngầm, Chỉ làm mới khi bấm nút, Nâng cao độ mượt mà và ổn định cho thiết bị."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.28)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.28&channel=stable'")
