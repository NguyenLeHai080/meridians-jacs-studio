...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Mở rộng giao diện Full-Width 100% không gian màn hình, Khắc phục triệt để hiện tượng lệch lề, Cân đối cân xứng các thẻ thống kê và bảng dữ liệu Credits."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.30)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.30&channel=stable'")
