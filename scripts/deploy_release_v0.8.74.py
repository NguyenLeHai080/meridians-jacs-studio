...
VERSION = "0.8.74"
NOTES = "Bản cập nhật v0.8.74: Tối ưu triệt để hiệu năng Bàn Dựng Timeline & Động cơ Playhead 60 FPS — Giảm 90% tải render giao diện, loại bỏ hoàn toàn hiện tượng đơ/treo máy khi phát hoặc kéo tua timeline; Nâng cấp thuật toán AI Stem Isolation phân mảnh bộ nhớ thấp (<50MB RAM) kèm thanh tiến độ thời gian thực (%) và ước tính thời gian còn lại (ETA); Giới hạn an toàn số luồng FFmpeg và hạ mức ưu tiên CPU tiến trình nền giúp hệ thống luôn mượt mà."
...
if os.path.exists(os.path.dirname(LOCAL_INSTALLED_ASAR)):
    try:
        shutil.copyfile(ASAR_PATH, LOCAL_INSTALLED_ASAR)
        print(f"✓ Đã sao chép app.asar trực tiếp vào cài đặt máy: {LOCAL_INSTALLED_ASAR}")
    except Exception as e:
        print(f"⚠️ Không thể sao chép trực tiếp vào Program Files (có thể app đang mở): {e}")

LOCAL_VOICE_WORKER = r"D:\Program Files\JACS Studio\resources\voice-runtime\voice_worker.py"
if os.path.exists(os.path.dirname(LOCAL_VOICE_WORKER)):
    try:
        shutil.copyfile(os.path.join(APP_DIR, "voice-runtime", "voice_worker.py"), LOCAL_VOICE_WORKER)
    except Exception as e:
        pass
