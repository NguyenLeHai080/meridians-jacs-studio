...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Xuất video ghép cắt chuẩn xác 100% theo các phân cảnh trên Timeline Studio, áp dụng đầy đủ cắt ghép clip và phụ đề."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.21)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.21&channel=stable'")
