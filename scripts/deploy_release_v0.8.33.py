...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Ra mắt Modal Nạp Credits & Quét Mã VietQR Động, Tách biệt hoàn toàn Cấu hình Gói Credit & Cấu hình Gói Model AI, Bảng quản lý đơn nạp và tự động cấp credits."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.32)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.32&channel=stable'")
