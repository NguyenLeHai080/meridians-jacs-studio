...
print(f"✅ Gói cập nhật OTA đã tạo thành công ({file_size / (1024*1024):.2f} MB)")
...
# Upload temp asar or run the remote update
...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Cập nhật bộ cài đặt NSIS hỗ trợ chọn ổ đĩa/thư mục cài đặt tùy ý (D:, E:...), tối ưu hóa trình gỡ cài đặt (Uninstaller) tự động đóng tiến trình và dọn dẹp sạch sẽ 100% dữ liệu/cache thừa."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.17&channel=stable'")
