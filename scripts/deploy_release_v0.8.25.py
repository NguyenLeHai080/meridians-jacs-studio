...
RELEASE_NOTES = f"Bản cập nhật v{VERSION}: Tự động xóa/tắt âm thanh gốc khi duyệt kịch bản sang Timeline và khi Render xuất video, chỉ giữ lại track 🎵 Voice Thuyết Minh."
...
print("\n🔍 Kiểm tra phản hồi từ API Server (Check update API cho v0.8.24)...")
stdin, stdout, stderr = ssh.exec_command("curl -s 'http://127.0.0.1:84/api/v1/releases/check?platform=windows&current_version=v0.8.24&channel=stable'")
