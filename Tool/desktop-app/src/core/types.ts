...
export type VideoProbe = { path: string; durationSeconds: number; width?: number; height?: number; fps?: number; sizeBytes?: number };
...
export type RenderResult = { outputPath: string; durationSeconds: number; passthrough?: boolean };
...
export type DesktopRuntime = {
  ...
  analyzeVideo?: (path: string, providerId?: string, operationId?: string) => Promise<AnalysisResult>;
  renderVideo?: (path: string, outputFolder?: string, options?: { mode?: string; startSeconds?: number; endSeconds?: number; aspectRatio?: Job["aspectRatio"]; preferredEngine?: ToolPreferences["preferredEngine"] }, operationId?: string) => Promise<RenderResult>;
