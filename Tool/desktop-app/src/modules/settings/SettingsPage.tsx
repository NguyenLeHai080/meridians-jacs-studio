...
import { DEFAULT_PREFERENCES, type ProviderDraft, type ProviderProfile, type ProviderType, type ToolPreferences } from "../../core/types";
...
function emptyProvider(type: ProviderType = "openai"): ProviderDraft {
  const capabilities = type === "custom" ? ["analysis"] : type === "openai" ? ["analysis", "vision", "transcription"] : ["analysis", "vision"];
...
export function SettingsPage({ preferences, onPreferencesChanged }: { preferences?: ToolPreferences; onPreferencesChanged?: (value: ToolPreferences) => void }) {
  ...
  const [updateState, setUpdateState] = useState<{ checking: boolean; message: string; release?: { version: string; release_notes: string; download_url: string; force_update: boolean } | null }>({ checking: false, message: "" });
  ...
  async function checkForUpdate() {
    ...
    if (!check) { setUpdateState({ checking: false, message: "Kiểm tra cập nhật cần chạy bản Electron." }); return; }
    setUpdateState({ checking: true, message: "Đang kiểm tra bản cập nhật..." });
    try {
      const result = await check("stable");
      if (!result.update_available || !result.release) {
        setUpdateState({ checking: false, message: "Bạn đang dùng phiên bản mới nhất." });
        return;
      }
      setUpdateState({ checking: false, message: `Có bản ${result.release.version} mới.`, release: result.release });
    } catch (error) {
      setUpdateState({ checking: false, message: error instanceof Error ? error.message : "Không kiểm tra được cập nhật" });
    }
  }
