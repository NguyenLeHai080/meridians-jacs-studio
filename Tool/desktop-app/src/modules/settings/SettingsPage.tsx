...
const PROVIDER_DEFAULTS: Record<ProviderType, { name: string; baseUrl: string; model: string; ttsModel?: string }> = {
  openai: { name: "OpenAI", baseUrl: "https://api.openai.com/v1", model: "gpt-4o-mini", ttsModel: "tts-1" },
  ...
  "openai-compatible": { name: "OpenAI Compatible", baseUrl: "https://api.example.com/v1", model: "gpt-5.6-sol", ttsModel: "tts-1" },
...
function emptyProvider(type: ProviderType = "openai"): ProviderDraft {
  const capabilities = type === "custom" ? ["analysis"] : ["openai", "openai-compatible"].includes(type) ? ["analysis", "vision", "transcription", "tts"] : ["analysis", "vision"];
...
export function SettingsPage({ preferences, onPreferencesChanged }: { preferences?: ToolPreferences; onPreferencesChanged?: (value: ToolPreferences) => void }) {
  ...
  function editProvider(profile: ProviderProfile) {
    setProviderError("");
    setProviderForm({ id: profile.id, name: profile.name, providerType: profile.providerType, baseUrl: profile.baseUrl, model: profile.model, transcriptionModel: profile.transcriptionModel, ttsModel: profile.ttsModel, apiKey: "", capabilities: profile.capabilities, enabled: profile.enabled });
  }
