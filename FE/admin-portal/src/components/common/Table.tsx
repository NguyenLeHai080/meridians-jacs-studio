...
export interface DataTableProps<T> {
  ...
  // Custom filter bar (e.g. Select dropdowns)
  ...
  return (
    ...
    >
      <div style={{ marginBottom: "0.75rem", opacity: 0.75 }}>
        {icon || <Inbox size={38} color="#cbd5e1" />}
      </div>
      <div style={{ fontSize: "0.95rem", fontWeight: 700, color: "#475569", marginBottom: "0.25rem" }}>
        {title}
      </div>
      {description && (
        <div style={{ fontSize: "0.8rem", color: "#94a3b8", maxWidth: "360px", marginBottom: action ? "1rem" : 0 }}>
          {description}
        </div>
      )}
...
export function Table({
  ...
  return (
    <div className="table-responsive" style={style}>
      <table className={`mf-table admin-table ${className}`}>{children}</table>
    </div>
  );
}
...
export function DataTable<T>({
  ...
  return (
    <div className={`mf-card-panel ${panelClassName} ${className}`}>
      {/* Top Header Row */}
      ...
      <div className="table-responsive">
        <table className={`mf-table admin-table ${tableClassName}`}>
          <thead>
            <tr>
              {columns.map((col) => {
                const headerText = col.header !== undefined ? col.header : col.title;
                return (
                  <th
                    key={col.key}
                    style={{
            ...
            {loading ? (
              <tr>
                <td colSpan={columns.length} style={{ textAlign: "center", padding: "3rem" }}>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: "0.6rem", color: "var(--primary)", fontWeight: 700, fontSize: "0.9rem" }}>
