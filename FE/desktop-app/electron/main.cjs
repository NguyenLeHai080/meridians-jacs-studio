...
function generateLocalStoryAnalysis(probe, customPrompt, language = "vi", options = {}, transcript = "", transcriptSegments = []) {
  ...
  for (let i = 0; i < targetCount; i++) {
    ...
    const fraction = targetCount <= 1 ? 0 : i / (targetCount - 1);

    let stageTitle = `Phân cảnh #${i + 1}`;
    let narrative = "";
    ...
    if (safeSegments.length > 0) {
      const matchingSegs = safeSegments.filter((seg) => {
        const s = Number(seg.start || 0);
        return s >= srcStartSec && s < srcEndSec;
      });
      if (matchingSegs.length > 0) {
        const segText = matchingSegs.map((s) => s.text).join(" ").trim();
        if (segText && segText.length > 15 && i > 0) {
          narrative = `${narrative} Cụ thể tại diễn biến này, lời thoại ghi nhận: "${segText.slice(0, 140)}"`;
        }
      }
    }
  ...
  return {
    ...
    summary: `Kịch bản phân tích và tóm tắt toàn diện (${targetDurMinutes} phút - ${targetCount} phân cảnh): Bóc tách toàn bộ cốt truyện và các điểm nhấn nổi bật của "${cleanTitle}" từ mở đầu đến hồi kết.`,
    scenes,
    score: 95,
    tokensUsed: 0,
    creditsUsed: 0,
...
function providerRequest(record, prompt, images = [], operationId, attempt = 0) {
  ...
  const maxOutputTokens = isGemini ? 16384 : 8192;
...
function expandScenesIfTooFew(scenes, targetCount, totalDuration, fallbackScenes, rawScript = "", cleanTitle = "", transcriptSegments = [], transcript = "") {
  ...
  for (let i = 0; i < safeTargetCount; i++) {
    ...
    const existing = cleanRawScenes[i];
...
async function renderVideoFile(event, filePath, folder, options = {}, operationId) {
  ...
  const renderWithCodec = (codec) => {
    ...
    const audioFilter = buildAudioFilter({
      ...
      originalAudioVolume: typeof options.originalAudioVolume === "number" ? options.originalAudioVolume : (options.narratorEnabled === false ? 100 : (options.interweaveAudio !== false ? 20 : 0)),
      musicVolume: options.backgroundMusicVolume ?? 20,
      narrationTempo,
      duckOriginalAudio: validNarration,
      autoDucking: options.autoDucking !== false,
      removeOriginalBgm: Boolean(options.removeOriginalBgm || options.isolateVocals),
      isolateVocals: Boolean(options.isolateVocals),
    });
    if (audioFilter) {
